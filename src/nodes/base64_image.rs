...
fn strip_data_url_prefix(data: &str) -> Result<&str, Box<dyn Error>> {
    // If the string starts with "data:" we trim everything up to the comma
    if let Some(comma) = data.find(',') {
        Ok(&data[comma + 1..])
    } else {
        // No header – assume the whole string is raw Base‑64
        Ok(data)
    }
}
